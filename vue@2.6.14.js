...
(function (global, factory) {
  ...
  function initMixin (Vue) {
    Vue.prototype._init = function (options) {
      //  this指向这个新实例对象，会从构造函数里继承某些属性和方法
      var vm = this;
      //  一个uuid
      vm._uid = uid$3++;
      ...
      //  这段if else 处理配置的合并
      //  @todo 暂时没有进入这段开关的情况，先跳过这个if
      if (options && options._isComponent) {
        // 优化内部组件实例化
        // 因为动态选项合并非常慢，并且没有任何内部组件选项需要特殊处理。
        initInternalComponent(vm, options);
      }
      //  常规合并配置
      else {
        //  resolveConstructorOptions，负责解析constructor上的options（可理解为一个默认值的配置）
